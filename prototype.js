...
Element.Methods = {
  ...
  previous: function(element, expression, index) {
    element = $prototype(element);
    if (arguments.length == 1) return $prototype(Selector.handlers.previousElementSibling(element));
  ...
  next: function(element, expression, index) {
    element = $prototype(element);
    if (arguments.length == 1) return $prototype(Selector.handlers.nextElementSibling(element));
  ...
  identify: function(element) {
    ...
    if (id) return id;
    do { id = 'anonymous_element_' + Element.idCounter++ } while ($prototype(id));
  ...
  getOffsetParent: function(element) {
    if (element.offsetParent) return $prototype(element.offsetParent);
    if (element == document.body) return $prototype(element);
...
if (Prototype.Browser.Opera) {
  ...
  Element.Methods.getOffsetParent = Element.Methods.getOffsetParent.wrap(
    function(proceed, element) {
      ...
      catch(e) { return $prototype(document.body) }
...
var Selector = Class.create({
  ...
  findElements: function(root) {
    ...
    switch (this.mode) {
      case 'selectorsAPI':
        if (root !== document) {
          var oldId = root.id, id = $prototype(root).identify();
          id = id.replace(/([\.:])/g, "\\$1");
          e = "#" + id + " " + e;
        }
...
Object.extend(Selector, {
  ...
  handlers: {
    ...
    id: function(nodes, root, id, combinator) {
      var targetNode = $prototype(id), h = Selector.handlers;
